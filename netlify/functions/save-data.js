...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        try {
            ...
            console.log('Processando documento para data:', data.date);
            const existing = await collection.findOne({ date: data.date });

            if (existing && data.update) {
                console.log(`Documento para ${data.date} já existe, atualizando...`);
                ...
                console.log('Documento já existe e não foi solicitado atualização');
                return {
                    statusCode: 409,
                    body: JSON.stringify({ error: 'Documento já existe para esta data' }),
                };
            }
        } catch (insertError) {
            console.log('Erro ao processar documento:', insertError.message);
            return {
                statusCode: 500,
